...
class MesinAbsensiController extends Controller
{
    ...
    public function processLogs(Request $request, MesinAbsensi $mesinabsensi)
    {
        ...
        foreach ($request->selected_logs as $logString) {
            try {
                list($pin, $datetime, $status) = explode('_', $logString);
                ...
                if (!isset($groupedLogs[$key])) {
                    $groupedLogs[$key] = [
                        ...
                        'check_ins' => [],
                        'check_outs' => [],
                        'mesin_id' => $mesinabsensi->id,
                    ];
                }

                if ($status == '0') { // Check-in
                    $groupedLogs[$key]['check_ins'][] = $dateTimeObj;
                } else if ($status == '1') { // Check-out
                    $groupedLogs[$key]['check_outs'][] = $dateTimeObj;
                }
            } catch (\Exception $e) {
                $errors++;
                $errorMessages[] = "Error parsing log ($logString): " . $e->getMessage();
            }
        }

        // Process grouped logs to find earliest check-in and latest check-out
        foreach ($groupedLogs as $key => $logData) {
            try {
                $pin = $logData['pin'];
                $date = $logData['date'];

                $karyawan = Karyawan::where('nik_karyawan', $pin)->first();
                if (!$karyawan) {
                    $errors++;
                    $errorMessages[] = "NIK tidak ditemukan: $pin";
                    continue;
                }
                ...
                // Find earliest check-in from the selected logs
                if (!empty($logData['check_ins'])) {
                    $earliestCheckIn = min($logData['check_ins']);
                    if (!$absensi->jam_masuk || $earliestCheckIn->lt($absensi->jam_masuk)) {
                        $absensi->jam_masuk = $earliestCheckIn;
                        $absensi->jenis_absensi_masuk = 'Mesin';
                        $absensi->mesinabsensi_masuk_id = $logData['mesin_id'];
                    }
                }

                // Find latest check-out from the selected logs
                if (!empty($logData['check_outs'])) {
                    $latestCheckOut = max($logData['check_outs']);
                    if (!$absensi->jam_pulang || $latestCheckOut->gt($absensi->jam_pulang)) {
                        $absensi->jam_pulang = $latestCheckOut;
                        $absensi->jenis_absensi_pulang = 'Mesin';
                        $absensi->mesinabsensi_pulang_id = $logData['mesin_id'];
                    }
                }

                // Recalculate total hours if both check-in and check-out exist
                if ($absensi->jam_masuk && $absensi->jam_pulang) {
                    if ($absensi->jam_pulang->gt($absensi->jam_masuk)) {
                        $totalDetik = $absensi->jam_pulang->diffInSeconds($absensi->jam_masuk);
                        $absensi->total_jam = $totalDetik;
                    } else {
                        $absensi->total_jam = 0;
                    }
                }
                ...
                $processedKaryawanHari++;

            } catch (\Exception $e) {
                $errors++;
                $errorMessages[] = "Error processing data for NIK ($pin) on date ($date): " . $e->getMessage();
            }
        }

        $message = "Berhasil memproses absensi untuk $processedKaryawanHari karyawan/hari.";
        if ($errors > 0) {
            $message .= " Ditemukan $errors error.";
            session()->flash('error_details', $errorMessages);
        }
